...
export class AuthService {
  ...
  async updateSettings(token: string, settings: SettingsInput): Promise<void> { const user = await this.authorize(token); await this.users.updateSettings(user.id, { user_id: user.id, notifications_enabled: settings.notificationsEnabled ? 1 : 0, language: settings.language, theme: settings.theme, privacy_level: settings.privacyLevel }); }
