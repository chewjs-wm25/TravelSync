1. Module IDs (MM): 
- 01=User & Account
- 02=Trip & Itinerary
- 03=Destination & Event
- 04=Logistics
- 05=Collaboration.
2. Prefixes: FR (Functional Requirement), UC (Use Case), UCD (Use Case Description).
3. Syntax Format: Group Level: `[PREFIX] [MM]-[Group#] [Title]` | Item Level: `[PREFIX] [MM]-[Group#].[Item#]: [Requirement]`
4. Spacing Rules: Always keep a space after PREFIX (e.g., `FR 03-1`, `FR 03-1.1:`).
5. Examples: Group: `FR 03-1 Search Location` | Sub-item: `FR 03-1.1: System shall...` | Use Case: `UC 03-1` / `UCD 03-1`.