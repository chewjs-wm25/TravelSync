...
### 类 `FavoritesService`
- 类型：类（构造函数注入 `repo: FavoritesRepository = sharedFavoritesRepository`、`routePlanner: RoutePlannerBridge = routePlannerBridge`——默认使用模块级单例，保证外部 `routePlannerBridge.setTargetItinerary()` 注入对实例生效）
