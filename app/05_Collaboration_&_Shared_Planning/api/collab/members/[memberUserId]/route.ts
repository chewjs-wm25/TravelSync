...
/** PATCH { role, tripId? } 修改角色 */
export async function PATCH(req: Request, ctx: Ctx) {
  try {
    const { memberUserId } = await ctx.params;
    const me = await resolveDemoUser(extractUserId(req));
    const body = (await req.json().catch(() => ({}))) as { role?: string; tripId?: string; trip_id?: string };
    const targetTripId = extractTripId(req, body);

    await requirePermission(targetTripId, me.id, "changeRole");
    ...
    const target = (await CollaboratorRepo.findByTrip(targetTripId)).find(
    ...
    await CollaboratorRepo.updateRole(targetTripId, memberUserId, role);
    await logActivity({
...
export async function DELETE(req: Request, ctx: Ctx) {
  try {
    ...
    await requirePermission(targetTripId, me.id, "removeMember");
    ...
    await CollaboratorRepo.deleteCollaborator(targetTripId, memberUserId);
    await logActivity({
      trip_id: targetTripId,
      user_id: me.id,
