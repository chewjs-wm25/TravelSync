...
import { Loader2, MapPin, Users, ShieldCheck, Share2, Lock, RefreshCw, CalendarDays } from "lucide-react";
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] transition hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
      ...
      <div className="mb-4 flex items-center gap-3 text-[11px] text-gray-500">
        ...
        {summary.myRole && (
          <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${summary.myRole === "Owner" ? "bg-primary-500/10 text-primary-500" : summary.myRole === "Editor" ? "bg-blue-50 text-blue-600" : "bg-gray-50 text-gray-500"}`}>
            {summary.myRole}
          </span>
        )}
      </div>
      ...
      <div className="mt-auto flex items-center gap-2">
        <a
          href={`/05_Collaboration_&_Shared_Planning?trip=${encodeURIComponent(summary.tripId)}`}
          target="_blank"
          rel="noopener noreferrer"
          onClick={(e) => {
            // 允许新 Tab，同时兼容同 Tab 切换（Ctrl/Cmd 点击已是新 Tab）
            // 阻止默认由父处理，改用 window.open 以确保多 Tab
            // 但保留 a 的 href 以支持右键新 Tab
          }}
      ...
      {isOwner && onToggle && (
        <div className="mt-3 flex items-center justify-between rounded-xl border border-gray-100 bg-[#FAF8FF] px-3 py-2.5">
          <span className="flex items-center gap-1.5 text-[11px] font-medium text-gray-600">
            {summary.isShared ? <Share2 size={12} className="text-emerald-500" /> : <Lock size={12} className="text-gray-400" />}
            {summary.isShared ? "Sharing enabled" : "Private only"}
          </span>
          <button
            disabled={toggling}
            onClick={() => handleToggle(!summary.isShared)}
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition ${summary.isShared ? "bg-emerald-500" : "bg-gray-300"} ${toggling ? "opacity-60" : ""}`}
            aria-label="Toggle share"
          >
            <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition ${summary.isShared ? "translate-x-4" : "translate-x-1"}`} />
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <p className="mt-2 text-xs leading-relaxed text-gray-500">
              This will <b className="text-red-600">immediately remove all collaborators</b> and expire pending invites. They will lose access instantly. This cannot be undone without re-inviting.
            </p>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Collaboration Control Center</h1>
          <p className="mt-1 text-sm text-gray-500">Choose which trip to share. Private trips are visible only to you; Shared trips can be collaborated.</p>
        </div>
        <button
          onClick={handleRefresh}
          disabled={refreshing}
          className="inline-flex items-center gap-1.5 self-start rounded-xl border border-gray-200 bg-white px-4 py-2 text-xs font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-60"
        >
      ...
      {controlError && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-600">{controlError}</div>
      )}

      <section className="space-y-4">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500">My Plans</h2>
          <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-500">{owned.length}</span>
        </div>
        {owned.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-8 text-center">
            <p className="text-sm font-medium text-gray-600">You have no trips yet.</p>
            <p className="mt-1 text-xs text-gray-400">Create a trip in Trip Planning first, then come back to enable sharing.</p>
            <a href="/02_Trip_Planning_&_Itinerary_Management" className="mt-4 inline-flex rounded-xl bg-primary-500 px-5 py-2 text-xs font-semibold text-white hover:bg-primary-500/80">
              Go to Trip Planning
            </a>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {owned.map((s) => (
              <TripCard key={s.tripId} summary={s} onToggle={handleToggle} onOpen={handleOpenSameTab} />
            ))}
          </div>
        )}
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500">Shared with me</h2>
          <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-600">{joined.length}</span>
        </div>
        {joined.length === 0 ? (
          <div className="rounded-2xl border border-gray-100 bg-gray-50 p-6 text-center text-xs text-gray-400">No shared trips yet. When someone invites you, accepted trips will appear here.</div>
        ) : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {joined.map((s) => (
              <TripCard key={s.tripId} summary={s} onOpen={handleOpenSameTab} />
            ))}
          </div>
        )}
      </section>
      ...
      <div className="rounded-2xl border border-gray-100 bg-[#FFFBF0] p-4 text-xs leading-relaxed text-gray-500">
        <b className="text-gray-700">Tip:</b> Each trip card’s <b>Open</b> opens in a <b>new tab</b> so you can collaborate on multiple Trips side-by-side. Same tab uses the same button’s secondary option. Private → Shared enables Invite; Shared → Private <b>immediately kicks all collaborators</b>.
      </div>
