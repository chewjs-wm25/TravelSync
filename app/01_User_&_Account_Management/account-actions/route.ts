...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (action === "verify-email") { const verificationToken = new URL(request.url).searchParams.get("token"); if (!verificationToken) throw new Error("Verification token is required"); await service.verifyEmail(verificationToken); return Response.json({ success: true, message: "Email verified. You can sign in now." }); }
