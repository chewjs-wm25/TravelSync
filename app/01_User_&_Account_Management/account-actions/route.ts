...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (action === "register") { const result = await service.register({ fullName: String(input.fullName ?? ""), email: input.email ? String(input.email) : undefined, phone: String(input.phone ?? ""), icNumber: String(input.icNumber ?? ""), password: String(input.password ?? ""), acceptTerms: input.acceptTerms === true }); const verificationUrl = result.verificationRequired ? `${new URL(request.url).origin}/01_User_&_Account_Management/account-actions?action=verify-email&token=${encodeURIComponent(result.verificationToken)}` : null; return Response.json({ user: result.user, verificationUrl, message: verificationUrl ? "Use the verification link below to activate your account." : "Account created. You can sign in with your phone number." }, { status: 201 }); }
